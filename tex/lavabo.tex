...
It is possible to use autocompletion to get the name of a board or to store the name of the board in an environment variable name \mintinline[bgcolor=grey]{bash}{LAVABO_BOARD} with the following line: \mintinline[bgcolor=grey]{console}{$ export LAVABO_BOARD=am335x-boneblack_01}
...
However, there are some possible improvments\footnote{\url{https://github.com/free-electrons/lavabo/issues}}. In the lab, there are some boards which can boot only when using a root filesystem mounted over NFS. However, there is no possibility yet to add an NFS directory to the server machine from lavabo. This problem prevents the use of these boards via lavabo. When following the typical workflow, engineers often forgot to stop \mintinline[bgcolor=grey]{console}{$ lavabo serial am335x-boneblack_01} which means they keep the serial connection open and nobody can use it even if the board is marked as available.
